import { forwardRef } from "react";
import type { SVGProps } from "react";
import clsx from "clsx";

export const IconCountryFlagSN = forwardRef<
  SVGSVGElement,
  SVGProps<SVGSVGElement>
>(function IconCountryFlagSN({ className, ...props }, ref) {
  return (
    <svg
      ref={ref}
      className={clsx("shrink-none", className)}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g clipPath="url(#clip0_161_33368)">
        <path
          d="M17.2175 1.19072C15.6399 0.427875 13.87 0 12.0001 0C10.1303 0 8.36037 0.427875 6.78274 1.19072L5.73926 12L6.78274 22.8093C8.36037 23.5721 10.1303 24 12.0001 24C13.87 24 15.6399 23.5721 17.2175 22.8093L18.261 12L17.2175 1.19072Z"
          fill="#FFDA44"
        />
        <path
          d="M12 7.82617L13.0359 11.0146H16.3888L13.6764 12.9854L14.7124 16.174L12 14.2033L9.28753 16.174L10.3237 12.9854L7.61133 11.0146H10.964L12 7.82617Z"
          fill="#496E2D"
        />
        <path
          d="M6.78262 1.19092C2.76872 3.13187 0 7.24229 0 11.9998C0 16.7573 2.76872 20.8677 6.78262 22.8086V1.19092Z"
          fill="#496E2D"
        />
        <path
          d="M17.2178 1.19141V22.8091C21.2317 20.8682 24.0004 16.7578 24.0004 12.0003C24.0004 7.24278 21.2317 3.13236 17.2178 1.19141Z"
          fill="#D80027"
        />
      </g>
      <defs>
        <clipPath id="clip0_161_33368">
          <rect width="24" height="24" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
});
IconCountryFlagSN.displayName = "IconCountryFlagSN";
