import { forwardRef } from "react";
import type { SVGProps } from "react";
import clsx from "clsx";

export const IconCountryFlagBO = forwardRef<
  SVGSVGElement,
  SVGProps<SVGSVGElement>
>(function IconCountryFlagBO({ className, ...props }, ref) {
  return (
    <svg
      ref={ref}
      className={clsx("shrink-none", className)}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g clipPath="url(#clip0_161_33440)">
        <path
          d="M24 12.0008C24 10.533 23.736 9.12688 23.2536 7.82694L12 7.30518L0.746391 7.82689C0.264 9.12688 0 10.533 0 12.0008C0 13.4687 0.264 14.8748 0.746391 16.1747L12 16.6965L23.2536 16.1748C23.736 14.8748 24 13.4687 24 12.0008Z"
          fill="#FFDA44"
        />
        <path
          d="M11.9997 23.9999C17.1593 23.9999 21.5578 20.7434 23.2533 16.1738H0.746094C2.44161 20.7434 6.84013 23.9999 11.9997 23.9999Z"
          fill="#6DA544"
        />
        <path
          d="M0.746094 7.8266H23.2534C21.5578 3.25699 17.1593 0.000488281 11.9997 0.000488281C6.84012 0.000488281 2.44161 3.25699 0.746094 7.8266Z"
          fill="#D80027"
        />
      </g>
      <defs>
        <clipPath id="clip0_161_33440">
          <rect width="24" height="24" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
});
IconCountryFlagBO.displayName = "IconCountryFlagBO";
