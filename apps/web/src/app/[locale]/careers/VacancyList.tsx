...
import { Button, SingleSelect } from "@grx/ui";
...
function JobCard({ vacancy }: { vacancy: HurmaVacancy }) {
  ...
  return (
    <Link
      ...
      className="group flex items-center gap-2 rounded-2xl border border-stroke-base-300 bg-surface-base px-6 py-6 transition-colors hover:border-stroke-strong-500 hover:bg-bg-weak-100"
    >
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        <div className="flex items-center gap-2">
          <span className="text-[16px] font-semibold leading-[24px] text-text-strong-1000">
            {vacancy.name}
          </span>
          {isHot && (
            <span className="shrink-0 rounded-[6px] bg-bg-soft-200 px-[6px] py-[2px] text-[12px] font-medium leading-[16px] text-text-strong-1000">
              HOT
            </span>
          )}
        </div>
            ...
            {metaItems.map((item, index) => (
              <span key={index} className="flex items-center gap-2">
                {index > 0 && (
                  <span
                    aria-hidden="true"
                    className="size-[4px] shrink-0 rounded-full bg-stroke-base-300"
                  />
                )}
                <span className="whitespace-nowrap text-[14px] font-normal leading-[20px] text-text-soft-500">
                  {item}
                </span>
              </span>
            ))}
      ...
      <div className="flex size-6 shrink-0 items-center justify-center rounded-[6px] bg-bg-weak-100 group-hover:bg-bg-soft-200">
        <ArrowRightIcon />
      </div>
    </Link>
  );
    ...
    <div className="flex flex-col gap-3">
      {title && (
        <div className="flex items-center gap-2 pb-4">
          <h2 className="text-[20px] font-bold leading-[24px] text-text-strong-1000">
            {title}
          </h2>
          {isHot && <FireIcon />}
        </div>
  ...
  return (
    <div className="mx-auto flex w-full max-w-[400px] flex-col items-center gap-10 text-center">
      ...
      <div className="flex w-full flex-col gap-2">
        <p className="text-[18px] font-semibold leading-[22px] text-text-strong-1000">
          {t("openPositions.noRolesTitle")}
        </p>
        <p className="text-[14px] font-normal leading-[20px] text-text-soft-500">
      ...
      <div className="flex w-full flex-col gap-3 sm:flex-row">
        <button
          type="button"
          onClick={onReset}
          className="flex-1 rounded-[10px] border border-primary-base-1000 px-4 py-[10px] text-[14px] font-semibold leading-[20px] text-text-strong-1000 transition-opacity hover:opacity-80"
        >
...
function NoPositionsState({ onSubmitCV }: { onSubmitCV: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-16 text-center">
      <p className="text-[20px] font-medium leading-[26px] text-text-soft-500">
        {t("openPositions.noPositions")}
      </p>
      <button
        type="button"
        onClick={onSubmitCV}
        className="inline-flex items-center rounded-[10px] bg-primary-base-1000 px-4 py-[10px] text-[14px] font-semibold leading-[20px] text-text-inverce transition-opacity hover:opacity-90"
      >
        {t("openPositions.submitCV")}
      </button>
    </div>
          ...
          <div className="flex flex-col gap-10">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between gap-4">
                <p className="text-[14px] leading-[20px] text-text-disabled">
                ...
                {hasFilters && (
                  <button
                    ...
                    className="shrink-0 rounded-[6px] px-2 py-1 text-[12px] font-semibold leading-[16px] text-text-strong-1000 transition-opacity hover:opacity-70"
                  >
